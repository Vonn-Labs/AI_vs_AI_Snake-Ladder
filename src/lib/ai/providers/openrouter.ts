...
export class OpenRouterProvider implements BaseAIProvider {
    ...
    constructor(apiKey: string, model: string = 'meta-llama/llama-4-scout') {
        this.apiKey = apiKey;
        this.model = model;
    }
    ...
    private async makeRequest(messages: Array<{ role: string; content: string }>, maxTokens: number): Promise<string> {
        const response = await fetch(`${this.baseUrl}/chat/completions`, {
            ...
            headers: {
                ...
                'HTTP-Referer': 'https://ai-snakeladder.vercel.app',
                'X-Title': 'AI Snake & Ladder',
            },
            body: JSON.stringify({
                model: this.model,
                messages,
    ...
    async generatePreRollCommentary(context: GameContext): Promise<string> {
        try {
            ...
            console.error('OpenRouter pre-roll error:', error);
    ...
    async generatePostRollCommentary(context: PostRollContext): Promise<string> {
        try {
            ...
            console.error('OpenRouter post-roll error:', error);
    ...
    async generateTrashTalk(context: PostRollContext): Promise<string> {
        try {
            ...
            console.error('OpenRouter trash talk error:', error);
